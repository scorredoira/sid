...
| `auto-reload` | Read a file again when something else changes it on disk. A buffer with unsaved changes is kept as it is, with a warning: `:reload` takes the file, `:w!` keeps the buffer | `true` |
